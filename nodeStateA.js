...
function ethTransaction(recipientAddr,energyAmount) {
  ...
  number.then(function(numb) {
      ...
      //var receivingAddr = new Buffer(recipientAddr,'hex'); // '0xb2d180dc3c55e57783c0351fd0279d779aa63286'
      var receivingAddr = recipientAddr;
      ...
      var txData = web3.utils.asciiToHex(ethAmount+' Ether for Energy');
 web3.eth.getTransactionCount(pubKey).then((nonce) => {
      var tx = new Tx({
          ...
          gasPrice: '0x22ECB25C00', // Normal is '0x14f46b0400' or 90 GWei
          gasLimit: '0x186A0', // Limit to be used by the transaction, default is '0x55f0' or 22000 GWei
      ...
      console.log(serializedTx.toString('hex')); // Log the resulting raw transaction hex for debugging if it fails to send
		console.log('About to send '+ethAmount+' to '+recipientAddr);
